...
export default function RTO() {
  ...
  const fetchRTODockets = async () => {
    setLoading(true);
    try {
      const result = await activityAPI.getRTO();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-800">RTO - Return to Origin</h1>
          <p className="text-gray-600 mt-1">All dockets being returned to sender</p>
        </div>
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">Loading RTO dockets...</p>
          </div>
        )}
        ...
        {!loading && filteredDockets.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            <p className="text-gray-500 text-lg">
              {searchQuery
                ? `No RTO dockets found matching "${searchQuery}"`
                : 'No RTO dockets found'}
            </p>
          </div>
        )}
