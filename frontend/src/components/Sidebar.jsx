...
import { ChevronDown, ChevronRight, Menu, X, LogOut, Lock, Truck, Package, PackageCheck, TruckElectric, SquareLibrary } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const { mobileOpen, closeSidebar } = useSidebarStore();
  ...
  const menuItems = {
    ...
    pod: {
      ...
      moduleIconClass: 'text-green-400',
      icon: ChevronDown,
      items: [
    ...
    coloader: {
      label: 'CO-LOADER',
      moduleIcon: TruckElectric,
      moduleIconClass: 'text-orange-400',
      icon: ChevronDown,
      items: [
    ...
    reports: {
      label: 'Reports',
      moduleIcon: SquareLibrary,
      moduleIconClass: 'text-purple-400',
      icon: ChevronDown,
      items: [
  ...
  // Render function (not a component) to avoid remount issues
  const renderContent = (isCollapsed, onClose) => (
    <>
      {/* Header */}
      <div className="p-4 border-b border-gray-700 flex items-center justify-between">
        <Link
          to="/"
          onClick={onClose}
          className={`flex items-center gap-2 ${isCollapsed && 'hidden'} hover:opacity-80 transition`}
        >
        ...
        <button
          onClick={onClose ? onClose : () => setSidebarOpen(!sidebarOpen)}
          className="p-1 hover:bg-gray-700 rounded"
        >
          {onClose ? <X size={20} /> : (sidebarOpen ? <X size={20} /> : <Menu size={20} />)}
        </button>
      </div>
      ...
      <nav className="p-2 space-y-1 flex-grow">
        {Object.entries(menuItems).map(([key, menu]) => {
          const ModuleIcon = menu.moduleIcon;
          return (
          <div key={key}>
            {menu.path ? (
              <Link
              ...
              >
                <span className={`flex items-center gap-2 ${isCollapsed && 'justify-center w-full'}`}>
                  {ModuleIcon && <ModuleIcon size={isCollapsed ? 20 : 16} className={menu.moduleIconClass} />}
              ...
              <>
                ...
                >
                  ...
                  {!isCollapsed && (expandedMenus[key] ? <ChevronDown size={18} /> : <ChevronRight size={18} />)}
                </button>

                {expandedMenus[key] && !isCollapsed && menu.items.length > 0 && (
      ...
      {/* Delivery Tracker Badge */}
      {showDeliveryBadge && (
        <div className="px-2 pb-2 relative z-50">
          <button
            onClick={() => { closeSidebar(); navigate('/delivery-tracker'); }}
            className={`
  ...
  return (
    <>
      {/* ── Mobile: backdrop — sits above navbar (z-50) ── */}
      {mobileOpen && (
        <div
      ...
      {/* ── Mobile: slide-in drawer — sits above backdrop and navbar ── */}
      <aside
        className={`
          fixed inset-y-0 left-0 z-[70] w-64
          bg-gray-800 text-gray-100 flex flex-col overflow-y-auto border-r border-gray-700
          transition-transform duration-300
          md:hidden
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}
        `}
      >
        {renderContent(false, closeSidebar)}
      </aside>

      {/* ── Desktop: normal sidebar (always visible, collapsible) ── */}
      <aside
        className={`
        ...
        `}
      >
        {renderContent(!sidebarOpen, null)}
      </aside>
    </>
  );
}
