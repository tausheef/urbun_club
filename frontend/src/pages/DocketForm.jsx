...
export default function DocketForm() {
  ...
  // ── Duplicate docket number check state ──
  // 'idle' | 'checking' | 'duplicate' | 'ok'
  const [docketNoStatus, setDocketNoStatus] = useState('idle');
  ...
  const fetchNextDocketNumber = async () => {
    try {
      ...
      if (result.success) {
        ...
        setDocketNoStatus('idle'); // auto-generated numbers are always fresh
      }
    } catch {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'docketNo') {
      ...
      setDocketNoStatus('idle'); // reset check status on every keystroke
    }
  ...
  // ── Check docket number uniqueness on blur ──
  const handleDocketNoBlur = async (e) => {
    const val = e.target.value.trim();
    if (!val) return; // let required validation handle empty
    setDocketNoStatus('checking');
    try {
      // Use docketAPI.search — handles dev/prod base URL automatically via axiosInstance
      const result = await docketAPI.search(val);
      // search returns dockets whose docketNo contains the query; filter for exact match
      const exactMatch = result?.data?.some(
        (item) => item.docket?.docketNo?.toLowerCase() === val.toLowerCase()
      );
      setDocketNoStatus(exactMatch ? 'duplicate' : 'ok');
      if (exactMatch) {
        setDocketErrors(prev => ({ ...prev, docketNo: `Docket No. "${val}" already exists` }));
      } else {
        setDocketErrors(prev => { const n = { ...prev }; delete n.docketNo; return n; });
      }
    } catch {
      setDocketNoStatus('idle'); // network error — don't block, backend will catch it
    }
  };
  ...
  const handleDocketSubmit = async (e) => {
    ...
    // ✅ Block submit if duplicate check shows this docket number already exists
    if (docketNoStatus === 'duplicate') {
      setMessage(`❌ Docket No. "${formData.docketNo.trim()}" already exists. Use a unique docket number.`);
      return;
    }

    // ✅ If user manually typed a docket number but hasn't blurred to trigger the check yet,
    // run the check now before submitting
    if (!isAutoGenerated && docketNoStatus === 'idle' && formData.docketNo.trim()) {
      setLoading(true);
      try {
        const result = await docketAPI.search(formData.docketNo.trim());
        const exactMatch = result?.data?.some(
          (item) => item.docket?.docketNo?.toLowerCase() === formData.docketNo.trim().toLowerCase()
        );
        if (exactMatch) {
          setDocketNoStatus('duplicate');
          setDocketErrors(prev => ({ ...prev, docketNo: `Docket No. "${formData.docketNo.trim()}" already exists` }));
          setMessage(`❌ Docket No. "${formData.docketNo.trim()}" already exists. Use a unique docket number.`);
          setLoading(false);
          return;
        }
        setDocketNoStatus('ok');
      } catch {
        // Network issue — let backend handle it
      }
      setLoading(false);
    }
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  onBlur={handleDocketNoBlur}
                  onKeyDown={handleKeyNav}
                  data-nav="10"
                  className={`w-full border ${
                    docketNoStatus === 'duplicate' || docketErrors.docketNo
                  ...
                  placeholder="053221"
                />
                {/* Duplicate / checking status feedback */}
                {docketNoStatus === 'checking' && (
                  <p className="text-blue-500 text-[11px] mt-0.5">⏳ Checking availability…</p>
                )}
                {docketNoStatus === 'duplicate' && (
                  <p className="text-red-500 text-[11px] mt-0.5 font-medium">
                    ⚠️ This Docket No. already exists — duplicates not allowed
                  </p>
                )}
                {docketNoStatus === 'ok' && !isAutoGenerated && (
                  <p className="text-green-600 text-[11px] mt-0.5">✅ Docket No. is available</p>
                )}
                <FieldError msg={docketNoStatus !== 'duplicate' ? docketErrors.docketNo : null} />
              </div>
